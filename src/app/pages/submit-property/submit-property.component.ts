...
export class SubmitPropertyComponent implements OnInit, OnDestroy {
  ...
  mobileValidationPattern= /^\+\d{1,3}-\d{3}-\d{3}-\d{4}$/;
  ...
  ngOnInit() {
    ...
    this.submitForm = this.fb.group({
      ...
      address: this.fb.group({
        ...
        propertyStatus: ['For Sale', Validators.required],
        // gallery: null,
        reraId: null
      }),
      ...
      additional: this.fb.group({
        ...
        built_major: ['Sq-ft', Validators.required],
        ageOfProperty: '',
  ...
  formSubmit() {
    if (this.displayAditionalFeaturPage) {
      ...
      this.submitForm.get('propertyFeature').updateValueAndValidity();
      this.submitForm.get('additional').updateValueAndValidity();
    } else {
    ...
    let finalArray = {
      ...
      // "features": this.submitForm.value.propertyFeature.features,
      ...
      "ownerDetails": this.submitForm.value.owner.owners, 
      ...
      "extra_details": {
        ...
        // "wishToEnterRd": this.submitForm.value.additional.wishToEnterRd,
        // "basic_price": this.submitForm.value.additional.basic_price,
        // "floor_plc": this.submitForm.value.additional.floor_plc,
        // "facing_plc": this.submitForm.value.additional.facing_plc,
        // "open_car_parking": this.submitForm.value.additional.open_car_parking,
        // "open_car_parking_per": this.submitForm.value.additional.open_car_parking_per,
        // "covered_car_parking": this.submitForm.value.additional.covered_car_parking,
        // "covered_car_parking_per": this.submitForm.value.additional.covered_car_parking_per,
        // "club_membership": this.submitForm.value.additional.club_membership,
        // "serviceTax": this.submitForm.value.additional.serviceTax,
        // "elecWaterCharge": this.submitForm.value.additional.elecWaterCharge,
        // "powerbackup": this.submitForm.value.additional.powerbackup,
        // "electricity": this.submitForm.value.additional.electricity,
        // "leaseRent": this.submitForm.value.additional.leaseRent,
        // "ifms": this.submitForm.value.additional.ifms,
        // "eecffc": this.submitForm.value.additional.eecffc,
        // "overlooking": this.submitForm.value.propertyFeature.overlooking,
        // "carParking": this.submitForm.value.propertyFeature.carParking,
        // "flooring": this.submitForm.value.propertyFeature.flooring,
        // "amenities": this.submitForm.value.propertyFeature.amenities,
      },//
    ...
    //     formData.append("doc_files", element.file);
    //   });

    // }
    // if (otherfiles !== null && otherfiles.length != 0) {
    //   otherfiles.forEach(element => {
    //     formData.append("other_files", element.file);
    //   });
    // }
    // // if(POA !== null){
    // //   formData.append("",POA);
    ...
    //     formData.append("reg_docs", element.file);
    //   });
    // }
    // if (bank !== null && bank.length != 0) {
    //   bank.forEach(element => {
    //     formData.append("bank_docs", element.file);
    //   });
    // }
    // if (completionCertificate !== null && completionCertificate.length != 0) {
    //   completionCertificate.forEach(element => {
    //     formData.append("comp_docs", element.file);
    //   });
    // }
    if (docfiles !== null && docfiles.length != 0) {
      docfiles?.forEach(element => {
        if(element.file){
          formData.append("doc_files", element.file);
          formData.append("docFlag", "true");
        }
        else
        { 
          formData.append("docFlag", "false");
        }
      });
    }else
    { 
    ...
    if (otherfiles !== null && otherfiles.length != 0) {
      otherfiles?.forEach(element => {
        if(element.file){
          formData.append("other_files", element.file);
          formData.append("otherDocFlag","true");
        }else{
          formData.append("otherDocFlag","false");
    ...
    if (societyRegCert !== null && societyRegCert.length != 0) {
      societyRegCert?.forEach(element => {
        if(element.file){
           formData.append("reg_docs", element.file);
           formData.append("regCertificateFlag","true");
        }else{
          formData.append("regCertificateFlag","false");
    ...
    if (bank !== null && bank.length != 0) {
      bank?.forEach(element => {
        if(element.file){
            ...
            formData.append("bank_docs", element.file);
           formData.append("bankDocFlag","true");

        }else{
          formData.append("bankDocFlag","false");
    ...
    if (completionCertificate !== null && completionCertificate.length != 0) {
      completionCertificate?.forEach(element => {
        if(element.file){
           formData.append("comp_docs", element.file);
           formData.append("compDocFlag","true");

        }else{
          formData.append("compDocFlag","false");
    ...
    formData.append("poaDocFlag","false");
  ...
  public createMobile(){
    return this.fb.control('',[Validators.pattern(this.mobileValidationPattern)]);
  ...
  public addDetail(index, mobOrEmail): void {
    if (mobOrEmail == 'mobile') {
      ...
      mobiles.push(this.createMobile());
  ...
  calculationMethod() {
    ...
    let finalValue = (Number(this.submitForm.value.additional.stampIncluded)) + (Number(this.submitForm.value.additional.otherCharges)) + (Number(this.submitForm.value.additional.regCharge)) + (Number(this.submitForm.value.additional.brokerageIncluded)) + Number(this.submitForm.value.additional.expectedPrice)
  ...
  addDescription() {
    ...
    this.expectedPrice = this.submitForm.value.additional.expectedPrice;
  ...
  getPropertyById(id) {
    ...
    this._pageService.getPropertyById(id).subscribe(res => {
      ...
      this.getBrokerageValue(formData.commercial_details.brokerage)
      ...
      let additional = {
        ...
        agreementCharges: formData.price_details.agreementCharges,
      ...
      this.submitForm.controls.owner.patchValue(owner);
      if(formData.poa_docs.length !== 0){
        this.submitForm.controls.owner['controls'].isPoa.setValue(true);
      }else{
        this.submitForm.controls.owner['controls'].isPoa.setValue(false);
      }
      formData?.ownerDetails?.forEach((ele,i)=>{
          if( i > 0){ 
            this.addOwners();
          }
          this.submitForm.controls.owner['controls'].owners.controls[i].controls.name.setValue(ele.name);
          ele.email.forEach((element,j) =>{
              if(j >= 0){
                this.addDetail(i,'email');
                this.submitForm.controls.owner['controls'].owners.controls[i].controls.email.controls[j].setValue(element)
              }
          });
          ele.mobile.forEach((element,k) => {
            if(k >= 0){
              this.addDetail(i,'mobile');
              this.submitForm.controls.owner['controls'].owners.controls[i].controls.mobile.controls[k].setValue(element);
            }
          });
          this.removeMobileOrEmail(i,-1,'email');
          this.removeMobileOrEmail(i,-1,'mobile');
      })
      ...
      formData.documentList.forEach(element => {
        this.listOfDocument.push(element);
      });

      formData.otherDocs.forEach(element => {
        this.index2.push(element);
      });

      formData.societyRegCertificate.forEach(element => {
        this.societyRegCert.push(element)

      });

      formData.bankDocs.forEach(element => {
        this.bankDocumnet.push(element);

      });
      formData.completionDocs.forEach(element => {
        this.completionCertificate.push(element);
      });
  ...
  getBrokerageValue(brokerage){
    ...
    let rent =   Number(this.submitForm["controls"].commercial['controls'].expectedPrice.value);
    let brokeragePrice = 0

   if(brokerageDays  == "0%"){
      brokeragePrice = 0;
    }else if(brokerageDays  == "0.25%"){
      brokeragePrice = rent * 0.25 / 100;
    }else if(brokerageDays  == "0.5%"){
      brokeragePrice = rent * 0.50 / 100;;
    }else if(brokerageDays  == "0.75%"){
      brokeragePrice = rent * 0.75 / 100; ;
    }else if(brokerageDays  == "1%"){
      brokeragePrice = rent * 1 / 100; ;
    }else if(brokerageDays  == "1.5%"){
      brokeragePrice = rent * 1.5 / 100; ;
    }else if(brokerageDays  == "2%"){
      brokeragePrice = rent * 2 / 100; ;
    }else if(brokerageDays  == "3%"){
      brokeragePrice = rent * 3 / 100; ;
    }
  ...
  calculatePriceSqFt(price){
    let expect = price.target.value;
    let carpet = this.submitForm["controls"].commercial['controls'].carpetArea.value;
    let pricePerSqft = expect / carpet;
    this.submitForm["controls"].commercial['controls'].expectedPricePerSqft.setValue(Math.round(pricePerSqft));
    this.calculateFinalPrice();
  }

  calculateFinalPrice(){

    let stampIncluded_value = ((Number(this.submitForm.value.commercial.expectedPrice) * Number(this.submitForm.value.commercial.stampDuty)) / 100)
    this.submitForm["controls"].commercial['controls'].stampIncluded.setValue(Math.round(stampIncluded_value));
    let regCharge = Number(this.submitForm["controls"].commercial['controls'].regCharge.value);
     
    let expected = Number(this.submitForm["controls"].commercial['controls'].expectedPrice.value);
    ...
    let stampDutyIncludedPrice = Math.round(Number(this.submitForm["controls"].commercial['controls'].stampIncluded.value));
    let final = expected + otherChages + booking + brockerage + stampDutyIncludedPrice + regCharge;
  ...
  deleteMedia(media_id,mediaArray){
    this._pageService.removeMediaFile(media_id).subscribe((res)=>{
      if(res["statusCode"] === 200){
        const objWithIdIndex = mediaArray.findIndex((obj) => obj.media_id === media_id);
        if (objWithIdIndex > -1) {
          mediaArray.splice(objWithIdIndex, 1);
        }
